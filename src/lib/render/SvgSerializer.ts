...
import type {
  ...
  StarElement, TextElement, PathElement
} from '@/types/elements'
...
function wrapG(inner: string, el: Element, anim: AnimatableProps): string {
...
function serializeRect(el: Element, anim: AnimatableProps): string {
  ...
  return wrapG(`<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="${rx}" ${fillStroke(el, anim)}/>`, el, anim)
}
...
function serializeCircle(el: Element, anim: AnimatableProps): string {
  ...
  const cx = r(x + w / 2)
  const cy = r(y + h / 2)
  return wrapG(`<circle cx="${cx}" cy="${cy}" r="${rv}" ${fillStroke(el, anim)}/>`, el, anim)
}
...
function serializeEllipse(el: Element, anim: AnimatableProps): string {
  ...
  return wrapG(`<ellipse cx="${r(x + w/2)}" cy="${r(y + h/2)}" rx="${r(w/2)}" ry="${r(h/2)}" ${fillStroke(el, anim)}/>`, el, anim)
}
...
function serializeLine(el: Element, anim: AnimatableProps): string {
  ...
  return wrapG(`<line x1="${r(x)}" y1="${r(y + h/2)}" x2="${r(x + w)}" y2="${r(y + h/2)}" stroke="${sc}" stroke-width="${r(sw)}" stroke-linecap="round"/>`, el, anim)
}
...
function serializePolygon(el: Element, anim: AnimatableProps): string {
  ...
  return wrapG(`<polygon points="${polygonPts(cx, cy, rv, e.sides ?? 6)}" ${fillStroke(el, anim)}/>`, el, anim)
}
...
function serializeStar(el: Element, anim: AnimatableProps): string {
  ...
  return wrapG(`<polygon points="${starPts(cx, cy, outerR, innerR, e.starPoints ?? 5)}" ${fillStroke(el, anim)}/>`, el, anim)
}
...
function serializePath(el: Element, anim: AnimatableProps): string {
  ...
  return wrapG(`<path d="${d}" fill="${fillVal}"${foAttr} ${scAttr}/>`, el, anim)
}

// ── Public API ────────────────────────────────────────────────────────────────

export function serializeElement(el: Element, anim: AnimatableProps): string {
...
export function serializeFrame(
  ...
  const shapes = elements
    ...
    .map(el => serializeElement(el, states.get(el.id) ?? {}))
    .join('\n')
  return (
