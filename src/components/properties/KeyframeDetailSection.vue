...
// Show detail for a single selected keyframe only
const selectedKf = computed<Keyframe | null>(() => {
  if (ui.selectedKeyframeIds.size === 1) {
    const id = [...ui.selectedKeyframeIds][0]
    return editor.keyframes.find(kf => kf.id === id) ?? null
  }
